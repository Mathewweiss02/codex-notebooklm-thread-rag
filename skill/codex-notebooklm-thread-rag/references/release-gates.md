...
- The final promotion step must run `scripts/thread_rag_benchmark_promote.py`, which requires all four scorer gates to be explicitly true and a passing causal audit bound to the exact score-report digest. A favorable aggregate without an independently supported explanation is not promotable.
