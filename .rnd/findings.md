...
- A fresh aggregate-only local candidate baseline on the current 145-thread projection recovered all 32/32 development positives at candidate depth 80 with zero local-search errors. This confirms the local discovery surface remains strong after corpus growth, but does not substitute for NotebookLM semantic recall or hybrid Top-1.
- An offline ranking-policy sweep against a recorded development run showed that stricter local acceptance thresholds can eliminate the observed 1/8 false-positive path only by causing broad positive abstention. No threshold variant was promoted; the result rules out blunt threshold tightening as the next fix.
- A one-case live source-scoped smoke after cooldown still received a classified NotebookLM rate-limit response and failed closed. This is operational evidence only and does not change retrieval-quality scores.
- The benchmark clients previously inherited the pinned library's automatic 429/5xx retry defaults. Both benchmark entry points now pass an explicit transport retry budget, defaulting to zero and recording it in reports; a follow-up one-case smoke failed closed after one rate-limit event without hidden transport backoff, so its latency and failure remain truthful operational evidence only.
- The isolated-ramp dry-run modeled the current 147 source parts across pool sizes 1, 2, 4, 8, 16, 32, and 50. Every modeled pool fits the configured 300-source/60-reserve and notebook-count limits, but the 50-worker case needs 49 new notebooks and 7,203 initial source-copy units; this is capacity planning, not live quota or quality proof.
